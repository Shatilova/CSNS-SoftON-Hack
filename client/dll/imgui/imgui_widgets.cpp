...
bool ImGui::HotKey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	const ImU32 col = GetColorU32(hovered ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	...
	RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding - ImVec2(0, 10), buf_display, NULL, NULL, style.ButtonTextAlign, &clip_rect);

	if (label_size.x > 0)
		RenderText(ImVec2(total_bb.Min.x + size.x + 4, frame_bb.Min.y + style.FramePadding.y), label);
